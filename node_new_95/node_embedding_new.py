...
from _megnet_new_95 import MEGNet
...
if __name__ == '__main__':
    ...
    # Training
    logger = CSVLogger("logs", name="MEGNet_training_no_nf__different_distance")

    trainer = pl.Trainer(max_epochs=1000, logger=logger, callbacks=[early_stop_callback])  # 指定gpus参数为1表示使用一块GPU进行训练
    trainer.fit(model=lit_module, train_dataloaders=train_loader, val_dataloaders=val_loader)

    # 保存模型
    save_path = "saved_models/x2o3_unembed_unequal_distance"
    metadata = {"description": "MEGNet trained using x2o3 node embedding with unequal distance",
