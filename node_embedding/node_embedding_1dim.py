...
if __name__ == '__main__':
    ...
    # Training
    logger = CSVLogger("logs", name="MEGNet_org_1dimension")
    trainer = pl.Trainer(max_epochs=1000, logger=logger, callbacks=[early_stop_callback])  # 指定gpus参数为1表示使用一块GPU进行训练
    trainer.fit(model=lit_module, train_dataloaders=train_loader, val_dataloaders=val_loader)

    # 保存模型
    save_path = "saved_models/node_original-1"
    metadata = {"description": "MEGNet trained using original node embedding 1 dimension",
